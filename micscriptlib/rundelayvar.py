...
def _get_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="rundelayvar",
        description="runs delayvar on existing rapidtide analyses",
        usage="%(prog)s",
    )
    parser.add_argument(
        "--inputdir",
    ...
    parser.add_argument(
        ...
        action="store",
        dest="extraargs",
        help="string containing extra arguments to use to invoke delayvar",
        default=None,
    )
    parser.add_argument(
        "--ncpus",
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="existcheck",
        help="run delayvar even if output file already exists",
        default=True,
    )
    parser.add_argument(
        "--alternateoutputdir",
...
def rundelayvar_workflow():
    ...
    if args.sourcetype == "cole":
        ...
        theoutputdir = os.path.join(outputroot, "derivatives", "delayvar")
        if args.task == "rest":
            thetypes = ["resting"]
        else:
    ...
    delayvarcmd = "/cm/shared/miniforge3/envs/mic/bin/delayvar"
    delayvarcmd = "delayvar"
    ...
    # put in the extra delayvar options
    if args.extraargs is not None:
        delayvaropts.append(args.extraargs)
    ...
    if args.volumeproc:
        ...
        delayvaropts.append("-c")
        outputnamesuffix = "grayordinate"
        qspec = ""

    delayvaropts.append(f"--outputlevel {args.outputlevel}")
    ...
    for thetype in thetypes:
        ...
        for datafileroot in therapidtideruns:
            ...
            for section in range(1):
                dothis = False
                if args.existcheck:
                    if os.path.isfile(theoutputroot + "_DELAYVARDONE.txt"):
                        print(theoutputroot + "_DELAYVARDONE.txt", "exists - skipping")
                        dothis = False
                    else:
                        print(theoutputroot + "_DELAYVARDONE.txt", "does not exist - running")
                        dothis = True
                else:
                    dothis = True
                ...
                scriptfile, thescript = micutil.make_runscript(
                    thiscommand, jobname="delayvar", timelimit=args.timelimit, mem=args.mem, ncpus=args.ncpus, debug=args.debug
                )
                if dothis:
                    if args.doforreal:
                        if SYSTYPE == "sge":
